...
def similarity(index, subject, comps):
    ...
    # index == 124:
    # print data.shape
    # print data.iloc[1:]['SALE_DATE', 'SCORE_SUM']

    # data.to_csv('output/comps/norm_comps_for_row_{}.csv'.format(index), index=False)
...
def main():
    ...
    result = Parallel(n_jobs=-1)(delayed(calc_saving)(i, row, df) for i, row in df.iterrows())
    ...
    output[output_cols].to_csv('output-town-25.csv', index=False)
